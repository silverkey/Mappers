# This script build the genomic annotations by using the GenomicFeatures package
# It is very fast, but you cannot save yet the structures. Therefore after saving the
# sqlite db you will have to reload it and to recalculate each time all the features
# (transcripts, promoters, exons, utr etc.). However this is not taking very long time....

# ----------------- #
#   JUST BEGINS..   #
# ----------------- #
# Lazy loading of a map table between ensembl ids and symbols
# because these info do not seem to be loaded by GenomicFeatures
# This will create into the workspace a data.frame variable named
# "map" with the pairings ensembl.gene.id <------> symbol
source('make.id.map_2.R')

# ----------------- #
#   RUNTIME VARS    #
# ----------------- #
# The directory and the file inside it to annotate
sel.dir = '/Users/remo/ANALYSIS/RIKEN/FINAL_SELECTED_DE/MAPPING_ANALYSIS/SECOND/20120611/PR/'
data = 'pr.all.coord.txt'
outfile = 'PR_RESULTS_FEATURE_ANNOTATED.xls'
# The directory containing the database or in which you will create it
dbdir = '/Users/remo/ANALYSIS/RIKEN/FINAL_SELECTED_DE/MAPPING_ANALYSIS/SECOND/DATA/'
genome = 'mm9'
tablename = 'ensGene'
download = 'F'
# Length of promoters
s.prox = 1000
e.prox = 400
# Columns containing the postional info in the files to annotate
# All the next fields are mandatory and id has to be unique!
id = 1
chr = 2
start = 3
end = 4
strand = 5

# ----------------- #
#     FUNCTIONS     #
# ----------------- #
get.promoters = function(transcripts,s.prox,e.prox) {
  pos = strand(transcripts) == '+'
  promoter.start = as.vector(ifelse(pos,start(transcripts)-s.prox,end(transcripts)-e.prox))
  promoter.end = as.vector(ifelse(pos,start(transcripts)+e.prox,end(transcripts)+s.prox))
  promoter.strand = ifelse(pos,'+','-')
  promoter.chr = seqnames(transcripts)
  promoter.tx_name = unlist(elementMetadata(transcripts[,'tx_name'])[,1])
  promoter.gene_id = unlist(elementMetadata(transcripts[,'gene_id'])[,1])
  promoters = GRanges(seqnames=promoter.chr,
                      ranges=IRanges(start=promoter.start,end=promoter.end),
                      strand=promoter.strand,
                      tx_name=promoter.tx_name,gene_id=promoter.gene_id)
  promoters
}

calculate.range.overlap = function(ranges,features,f.name,f.dir) {
  f.ov2 = findOverlaps(ranges,features,select='first')
  rows = 1:length(f.ov2)
  f.ov = data.frame(row.names=rows)
  f.ov[,1] = rows
  f.ov[,2] = f.ov2
  f.ov = na.omit(f.ov)
  id = as.character(elementMetadata(ranges[f.ov[,1]])$id)
  f.gene = as.character(elementMetadata(features[f.ov[,2]])$gene_id)
  f.trans = as.character(elementMetadata(features[f.ov[,2]])$tx_name)
  f.strand = as.character(strand(features[f.ov[,2]]))
  res = as.data.frame(unique(cbind(id,f.name,f.gene,f.trans,f.strand,f.dir)))
  if(ncol(res)<6) no.res = ranges
  if(ncol(res)==6) no.res = ranges[-f.ov[,1]]
  list(res=res,no.res=no.res)
}

calculate.rangelist.overlap = function(ranges,features,f.name,txid.gid,f.dir) {
  f.ov2 = findOverlaps(ranges,features,select='first')
  rows = 1:length(f.ov2)
  f.ov = data.frame(row.names=rows)
  f.ov[,1] = rows
  f.ov[,2] = f.ov2
  f.ov = na.omit(f.ov)
  id = as.character(elementMetadata(ranges[f.ov[,1]])$id)
  f.gene = as.character(txid.gid[names(features[f.ov[,2]]),2])
  f.trans = as.character(txid.gid[names(features[f.ov[,2]]),1])
  f.strand = as.character(txid.gid[names(features[f.ov[,2]]),3])
  res = as.data.frame(unique(cbind(id,f.name,f.gene,f.trans,f.strand,f.dir)))
  if(ncol(res)<6) no.res = ranges
  if(ncol(res)==6) no.res = ranges[-f.ov[,1]]
  list(res=res,no.res=no.res)
}

antisense.strand = function(strand) {
  a = 'NA'
  if(strand == '+') a = '-'
  if(strand == '-') a = '+'
  if(strand == '1') a = '-1'
  if(strand == '-1') a = '1'
  if(a == 'NA') stop(paste('Cannot recognize strand: ',strand,sep=''))
  a
}

granges.antisense = function(ranges) {
  strand = as.data.frame(ranges)$strand
  strand(ranges)=Rle(unlist(lapply(strand,antisense.strand)))
  ranges
}

# ----------------- #
#      SCRIPT       #
# ----------------- #
library("GenomicFeatures")

if(download == 'T') {
  transdb = makeTranscriptDbFromUCSC(genome=genome,tablename=tablename)
  saveFeatures(transdb,file=paste(dbdir,'/',genome,'.',tablename,'.','sqlite',sep=''))
} else {
  transdb = loadFeatures(file=paste(dbdir,'/',genome,'.',tablename,'.','sqlite',sep=''))
}

setwd(sel.dir)

# Completely noncodings transcripts will overlap exons but not cds nor utr
# We use a hierarchy order for which every time a range is overlapping a feature than
# it is associated to the feature and cut-out from the ranges, so that it cannot overlapping
# with the next order feature.
# Order to test overlap:
# 1) promoters
# 2) utr
# 3) cds
# 4) exon
# 5) intron

# Build Features
transcripts = transcripts(transdb,columns=c("tx_id","tx_name","gene_id")) # range
cds = cdsBy(transdb,by='tx',use.names=T)                                  # rangelist
exons = exonsBy(transdb,by='tx',use.names=T)                              # rangelist
introns = intronsByTranscript(transdb,use.names=T)                        # rangelist
utr5 = fiveUTRsByTranscript(transdb,use.names=T)                          # rangelist
utr3 = threeUTRsByTranscript(transdb,use.names=T)                         # rangelist
promoters = get.promoters(transcripts,s.prox,e.prox)                      # range

# Build a comfortable table to associate transcripts->genes->strands
txid.gid = as.data.frame(cbind(
                         unlist(elementMetadata(transcripts[,'tx_name'])[,1]),
                         unlist(elementMetadata(transcripts[,'gene_id'])[,1]),
                         as.character(strand(transcripts))))
colnames(txid.gid) = c('tx_id','gene_id','strand')
rownames(txid.gid)=txid.gid$tx_id

source.tab = read.delim(file=data)
ncol.f = ncol(source.tab)
selected = read.delim(file=data)[,c(id,chr,start,end,strand)]
colnames(selected) = c('id','chr','start','end','strand')

features.res.cn = c('id','f.name','f.gene','f.trans','f.strand','f.dir')
features.res = matrix(ncol=length(features.res.cn),nrow=0)
features.tab = matrix(ncol=ncol.f+length(features.res.cn)-1,nrow=0)
colnames(features.res) = features.res.cn

# Build the ranges of the fragments you want to test the overlap with features
ranges = GRanges(seqnames=Rle(selected$chr),
                 ranges=IRanges(start=selected$start,end=selected$end),
                 strand=Rle(selected$strand),
                 id=selected$id)
  
lres = calculate.range.overlap(ranges,promoters,'promoter','sense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,utr5,'utr5',txid.gid,'sense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,utr3,'utr3',txid.gid,'sense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,cds,'cds',txid.gid,'sense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,exons,'exon',txid.gid,'sense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,introns,'intron',txid.gid,'sense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

ranges = granges.antisense(ranges)

lres = calculate.range.overlap(ranges,promoters,'promoter','antisense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,utr5,'utr5',txid.gid,'antisense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,utr3,'utr3',txid.gid,'antisense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,cds,'cds',txid.gid,'antisense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,exons,'exon',txid.gid,'antisense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

lres = calculate.rangelist.overlap(ranges,introns,'intron',txid.gid,'antisense')
ranges = lres$no.res
if(ncol(lres$res)==ncol(features.res)) features.res = rbind(features.res,lres$res)

intergenic = as.data.frame(lres$no.res)
if(nrow(intergenic)>=1) {
  intergenic$f.name='intergenic'
  intergenic$f.gene=NA
  intergenic$f.trans=NA
  intergenic$f.strand=NA
  intergenic$f.dir=NA
  features.res = rbind(features.res,intergenic[,features.res.cn])
}

features.tab = rbind(features.tab,merge(source.tab,features.res,by.x=id,by.y='id',all.x=T))
col.order = c(colnames(features.tab),'sym')
features.tab = merge(features.tab,sym.map,by.x='f.gene',by.y='ens',all.x=T)
features.tab = features.tab[,col.order]
features.tab = merge(features.tab,bio.map,by.x='f.trans',by.y='tra',all.x=T)
newpos = ncol(source.tab)+1+1+1
features.tab = features.tab[,c(2:newpos,1,(newpos+1):ncol(features.tab))]
write.table(unique(features.tab),file=outfile,sep="\t",row.names=F,quote=F)

# CHECK IF THE ANTISENSE ARE ENRICHED IN A SPECIFIC LOCATION
#sense=table(features.tab[features.tab$f.dir=='sense',]$f.name)[c('promoter','utr5','cds','exon','intron','utr3')]
#anti=table(features.tab[features.tab$f.dir=='antisense',]$f.name)[c('promoter','utr5','cds','exon','intron','utr3')]
#intergenic=table(features.tab$f.name)['intergenic']
#tot.sense=sum(table(features.tab[features.tab$f.dir=='sense',]$f.name)[c('promoter','utr5','cds','exon','intron','utr3')])
#tot.anti=sum(table(features.tab[features.tab$f.dir=='antisense',]$f.name)[c('promoter','utr5','cds','exon','intron','utr3')])
#c=cbind(sense,anti,tot.sense,tot.anti)
#test = function(x) { prop.test(c(x[1],x[2]),c(x[3],x[4]),alternative='l')$p.value }
#apply(c,1,test)
#par(mfrow=c(1,2))
#barplot(sense/tot.sense*100,ylim=c(0,55),main=paste('Percentage DE clusters overlapping genes in sense orientation (n=',tot.sense,')',sep=''))
#barplot(anti/tot.anti*100,ylim=c(0,55),main=paste('Percentage DE clusters overlapping genes in antisense orientation (n=',tot.anti,')',sep=''))

#sense=table(features.tab[features.tab$f.dir=='sense',]$bio)
#anti=table(features.tab[features.tab$f.dir=='antisense',]$bio)
#tot.sense=sum(table(features.tab[features.tab$f.dir=='sense',]$bio))
#tot.anti=sum(table(features.tab[features.tab$f.dir=='antisense',]$bio))
#c=cbind(sense,anti,tot.sense,tot.anti)
#test = function(x) { prop.test(c(x[1],x[2]),c(x[3],x[4]),alternative='l')$p.value }
#apply(c,1,test)
#par(mfrow=c(1,2))
#barplot(sense/tot.sense*100,ylim=c(0,55),main=paste('Percentage DE clusters overlapping genes in sense orientation (n=',tot.sense,')',sep=''))
#barplot(anti/tot.anti*100,ylim=c(0,55),main=paste('Percentage DE clusters overlapping genes in antisense orientation (n=',tot.anti,')',sep=''))

